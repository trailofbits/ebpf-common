...
StringErrorOr<IEvent::Ref>
IEvent::createRawTracepoint(const std::string &name) {
  try {
    return Ref(new RawTracepointEvent(name));
...
StringErrorOr<IEvent::Ref> IEvent::createLSM() {
  try {
    return Ref(new RawTracepointEvent(std::nullopt));
